...
from tasks.matching.core.match_roma import keypoint_matcing_RoMa
...
def task_matching(params):
    ...
    if matcher_type == "LightGlue":
        ...
        keypoints, descriptors = detect_keypoints_dedode(
            ...
            W=dedode_W,
            rects=rects,
            dynamic_resize=params.get("keypoint_detection_args", {}).get("dynamic_resize", None),
            device=params["device"],
        )

        # Free detector/descriptor GPU memory
        ...
        matches = keypoint_matching_dedode(
            ...
            min_matches=matching_args.get("min_matches", 15),
        ...
        # PostProcess(keypoints) — revert rotation + crop, same logic as LightGlue
        offsets = {}
        keypoints = {}
        with h5py.File(keypoints_h5_path, mode="r") as f_keypoints:
            for data in image_paths:
        ...
        # Matching keypoints using LightGlue (dedodeb)
        dir_img1_list = [p1+" "+str(p2) for p1, p2 in zip(img1_list, dir1_list)]
        dir_img2_list = [p1+" "+str(p2) for p1, p2 in zip(img2_list, dir2_list)]
        image_pairs = list(zip(dir_img1_list, dir_img2_list))
        ...
        # PostProcess(keypoints) — revert rotation + crop, same logic as DeDoDe branch
        offsets = {}
        keypoints = {}
        with h5py.File(keypoints_h5_path, mode="r") as f_keypoints:
            for data in image_paths:
        ...
        # RoMa pair-wise dense matching (same pattern as DKM)
        images_dir = params["data_dict"][0].parent
        image_pairs = list(zip(image_pair_df["key1"].values.tolist(), image_pair_df["key2"].values.tolist()))
        dir_pairs = list(zip(image_pair_df["dir1"].values.tolist(), image_pair_df["dir2"].values.tolist()))

        roma_args = params["roma_args"]
        keypoints, matches = keypoint_matcing_RoMa(
            image_pairs,
            dir_pairs,
            images_dir,
            roma_args=roma_args,
            max_matching_num=params.get("keypoint_matching_args", {}).get("max_matching_num", 10000),
            sample_thresh=params.get("keypoint_matching_args", {}).get("sample_thresh", 0.05),
            min_matches=params.get("keypoint_matching_args", {}).get("min_matches", 15),
            rects=rects,
            device=params["device"],
        )
        ...
        # LoMa pair-wise matching (DaD detector + DeDoDe descriptor + learned matcher)
        images_dir = params["data_dict"][0].parent
        image_pairs = list(zip(image_pair_df["key1"].values.tolist(), image_pair_df["key2"].values.tolist()))
        dir_pairs = list(zip(image_pair_df["dir1"].values.tolist(), image_pair_df["dir2"].values.tolist()))
        ...
        matching_args = params.get("keypoint_matching_args", {})
        keypoints, matches = keypoint_matching_LoMa(
            image_pairs,
            dir_pairs,
            images_dir,
            loma_args=loma_args,
            num_keypoints=matching_args.get("num_keypoints", 2048),
            filter_threshold=matching_args.get("filter_threshold", 0.1),
            min_matches=matching_args.get("min_matches", 15),
            rects=rects,
            device=params["device"],
        )
