...
def _load_dedodeb_lightglue_matcher(matcher_params, weights_path=None, device="cpu"):
    """Load LightGlue matcher for DeDoDe-B, with offline weight support.

    If weights_path is provided, pre-caches the weights so that
    KF.LightGlueMatcher does not attempt to download them.
    """
    import os as _os
    if weights_path is not None:
        # Pre-cache: copy/symlink to torch hub cache so LightGlue finds it
        import shutil
        cache_dir = torch.hub.get_dir()
        hub_cache = _os.path.join(cache_dir, "checkpoints")
        _os.makedirs(hub_cache, exist_ok=True)
        cached_path = _os.path.join(hub_cache, "dedodeb_lightglue.pth")
        if not _os.path.exists(cached_path):
            shutil.copy2(weights_path, cached_path)
            print(f"[LightGlue] Cached weights: {weights_path} -> {cached_path}")
...
def keypoint_matching_dedode_lightglue(
    ...
    lightglue_weights_path: str = None,
    min_matches: int = 15,
    verbose: bool = False,
    device: torch.device = torch.device("cpu"),
):
    """Match DeDoDe descriptors using LightGlue (dedodeb checkpoint).

    Uses KF.LightGlueMatcher with the 'dedodeb' feature type, which expects
    256-dim descriptors matching DeDoDe Descriptor-B output.

    Args:
        lightglue_weights_path: Path to local dedodeb_lightglue.pth file.
            Required in offline environments (e.g. Kaggle). If None, falls
            back to automatic download.
    Output format: dict[key1][key2] = ndarray [K, 2] of index pairs,
    identical to keypoint_matcing_LG and keypoint_matching_dedode.
    """
    ...
    matcher = _load_dedodeb_lightglue_matcher(matcher_params, lightglue_weights_path, device)
    matches = {}

    with h5py.File(keypoints_h5_path, mode="r") as f_keypoints, \
         h5py.File(descriptions_h5_path, mode="r") as f_descriptors:
        for key1, key2 in tqdm(image_pairs, desc="Matching (DeDoDe+LightGlue)"):
            keypoints1 = torch.from_numpy(f_keypoints[key1][...]).to(device)
            keypoints2 = torch.from_numpy(f_keypoints[key2][...]).to(device)
            descriptors1 = torch.from_numpy(f_descriptors[key1][...]).to(device)
            descriptors2 = torch.from_numpy(f_descriptors[key2][...]).to(device)
